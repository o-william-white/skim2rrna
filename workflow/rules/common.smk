...
def get_plot_tree_output(wildcards):
    ...
    return expand("results/plot_tree/{i}.png", i=glob_wildcards(os.path.join(checkpoint_output, "{i}.fasta")).i)

def get_mafft_filtered_output(wildcards):
    checkpoint_output = checkpoints.extract_annotated_genes.get(**wildcards).output[0]
    return expand("results/mafft_filtered/{i}.fasta", i=glob_wildcards(os.path.join(checkpoint_output, "{i}.fasta")).i)
...
if barrnap_kingdom not in ["bac","arc","euk"]:
