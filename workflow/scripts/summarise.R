...
annotations_files <- list.files("results/annotations/", recursive = T, pattern = "result.gff", full.names = T)

# read as list
annotations_list <- lapply(annotations_files, function(x) {
  read.table(x, header = F, sep = "\t",
             col.names = c("sequence", "source", "feature", "start", "end", "score", "strand", "frame", "attribute") )
})
...
# add sample names
annotations_dat <- cbind(ID = gsub("_circular$|_contig\\d*$","", annotations_dat$sequence),
                         name = gsub("Name=", "", sapply(strsplit(annotations_dat$attribute, ";"), function(x) x[1])),
                         annotations_dat)
